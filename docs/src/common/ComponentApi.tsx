...
const ComponentApi: React.FunctionComponent<{ metadata: any; exportedBy: any }> = ({
  metadata,
  exportedBy,
}) => {
  ...
  return (
    <>
      ...
      <Code aria-label={`Import code for the ${name} component`}>
        <Keyword>import</Keyword> {name} <Keyword>from</Keyword> `@trimbleinc/modus-react-bootstrap/
        {name}`
      </Code>
      {/* use composes here */}
